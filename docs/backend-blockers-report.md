...
| **BE-I-32** | Linking a student from their result to their own answer review | `submit`/`late-submit` resolve to `ScoreResult` `{ score, passed, correctCount, totalCount }` with **no `attemptId`** (`exam.service.ts:71-76`), but `GET /exam/attempts/:attemptId/review` is keyed by one. Filed 2026-08-01. | Shipped **routed off the attempt history** instead: `/assessments/review/:attemptId`. Guessing the newest attempt was rejected — wrong under retakes and concurrent attempts. |
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **BE-I-30** ✅       | _(inverse case)_ `66a7632` **deleted `GET /landing`** (`LandingController` + `landing-response.dto.ts` removed); replacement is `GET /analytics/public-stats` → `{ stats:{ programs, students, certificatesIssued } }` (`analytics/public-stats.controller.ts:20-31`), with featured programs from `GET /catalog` and static content from `GET /cms/pages/home`. | ✅ **DONE 2026-08-01** — `LandingApi` repointed to `GET /analytics/public-stats`; featured programs composed from the shared `PublicCatalogStore`. No `/landing` call remains in `src/`. **Correction to the old wording here:** the page never visibly 404'd — both consumers (`store.stats()`, `store.featuredPrograms()`) had been commented out in `f3c425d`, so the symptom was one failing background request per load, not a broken page. Done **without** any CMS work (plan Slice 1 satisfied; the Slice 8 CMS home cutover is untouched). |
| **BE-I-22**          | ✅ Fixed `66a7632` — `GET /exam/attempts/:attemptId/review` (`exam.controller.ts:224-249`): owner-only, terminal attempts only (422 otherwise), returns `options[].isCorrect`, `selectedOptionId`, `correctOptionId`, per-question `isCorrect`, `explanation`.                                                                                                  | ✅ **DONE 2026-08-01** — `ios-exam-review-page` at `/assessments/review/:attemptId` + transport in `exam.api.ts`. **Not** inlined on the result page: that route is keyed by `sessionId` and submit returns no attempt id — see **BE-I-32**.                    |
