...
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `src/app/features/dashboard/components/user-menu-dropdown.ts` | Duplicate of `src/app/layouts/app-shell/user-menu-dropdown.ts` — same `ios-user-menu-dropdown` selector, older draft without the locale-switcher block. Never imported by any page; only referenced by the equally-orphaned sibling `features/dashboard/components/dashboard-navbar.ts`.                                    | File deleted. The layout-shell copy is the canonical one.                                                      |
| `src/app/features/dashboard/components/dashboard-navbar.ts`   | Duplicate of `src/app/layouts/app-shell/dashboard-navbar.ts` — same `ios-dashboard-navbar` selector. **Every** page that needs the dashboard navbar imports the layout-shell version via `@layouts` (13 pages: dashboard, certificates, profile, settings, notifications, assessments). Zero outside imports for this copy. | File deleted.                                                                                                  |
...
| `src/app/features/dashboard/components/user-menu-dropdown.ts`   | **DELETED** — orphaned duplicate of `layouts/app-shell/user-menu-dropdown.ts` (same `ios-user-menu-dropdown` selector, never imported).                                 |
| `src/app/features/dashboard/components/dashboard-navbar.ts`     | **DELETED** — orphaned duplicate of `layouts/app-shell/dashboard-navbar.ts` (same `ios-dashboard-navbar` selector, never imported; all 13 caller pages use `@layouts`). |
...
**Net diff:** 11 files modified, 2 new files, 3 files deleted, 1 directory removed.
...
4. **Three duplicate components eliminated.** Deleted the unused second `LogoutDialog` (same `ios-logout-dialog` selector), the unused second `UserMenuDropdown` (same `ios-user-menu-dropdown` selector), and the unused second `DashboardNavbar` (same `ios-dashboard-navbar` selector). All three lived under `features/.../components/` but were never imported — every caller uses the canonical copies under `layouts/app-shell/` via the `@layouts` barrel. Three latent selector-collision bugs gone.
...
6. **One lint error → zero.** The dead `MENU_ITEMS` const that triggered it was inside the now-deleted duplicate `user-menu-dropdown.ts`.
